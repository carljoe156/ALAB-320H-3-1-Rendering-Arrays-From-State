...
function App() {
  const [learnerData] = useState({
    // Using useState for our Learners and Scores
    learners: [
      {
        name: "Cait Yomorta",
  ...
  return (
    <div className="App">
      {learnerData.learners.map(
        (
          learner //For mapping over the learners and rendering the relevant information for each learner
        ) => (
          <Learner key={learner.name} learner={learner} /> // Using the object learner as a prop to get the learner component
        )
      )}
    </div>
  );
